...
export function generateAuditReport(
  ...
  if (extInputRPM !== null && extInputRPM !== undefined && extInputRPM > 0) {
    ...
    const speed = syncSpeedRPM * (1 - ENGINEERING_CONSTANTS.AVG_MOTOR_SLIP); // actual speed assuming avg motor slip
...
${notes}`);
  ...
  const getLineageNode = <T,>(key: string, defaultNode: AuditParameterNode<T>): AuditParameterNode<T> => {
    if (lineage && lineage[key] && lineage[key].value !== null) {
      ...
      return {
        ...
        powerSide: defaultNode.powerSide
      };
    }
    return defaultNode;
  };
  ...
  const powerNode = getLineageNode<number>('powerKW', powerSide === 'OUTPUT' ? {
    name: 'Input Power (kW)',
    value: resolvedPower!,
    type: 'CALCULATED',
